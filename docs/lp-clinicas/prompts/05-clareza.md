# 05 — Entender o que funciona

- **Uso:** etapa 04 — decidir com clareza.
- **Arquivo final no projeto:** `public/assets/clinicas/05-clareza.webp`.
- **Chave em `src/pages/Clinicas.jsx`:** `IMAGES.clarity`.
...
CENA
Uma gestora adulta brasileira está em uma pequena sala de apoio da mesma clínica, revisando informações em um notebook e fazendo uma anotação em caderno sem texto legível. Expressão de atenção e tranquilidade. Pela porta ou divisória de vidro, é possível reconhecer os materiais da recepção: madeira clara, parede marfim e iluminação quente. Ambiente de trabalho real e organizado, com poucos objetos.

COMPOSIÇÃO
Retrato ambiental em plano médio, câmera na altura dos olhos. Pessoa, notebook e gesto ocupam os 70% centrais. A tela fica de lado e fora de foco. Não criar dashboards, gráficos ascendentes, números de faturamento, percentuais ou resultados. A fotografia deve comunicar entendimento e decisão, com atmosfera humana e sem estética de escritório corporativo genérico.
...
- A cena comunica “entender o que funciona” sem precisar de uma legenda técnica?
...
Salve a variante como `05-clareza-mobile.webp`. A primeira versão da LP usa recorte CSS; a variante mobile requer inclusão em um elemento `<picture>` quando integrada.
