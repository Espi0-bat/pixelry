# 04 — Facilitar o contato

- **Uso:** etapa 03 — facilitar o contato.
- **Arquivo final no projeto:** `public/assets/clinicas/04-contato.webp`.
- **Chave em `src/pages/Clinicas.jsx`:** `IMAGES.contact`.
...
CENA
Na mesma recepção da referência, uma recepcionista adulta está atendendo uma mensagem no celular atrás do balcão de madeira clara. Expressão concentrada e acolhedora, postura natural. Um computador discreto aparece ao fundo com tela desfocada. A imagem representa o início de uma conversa, sem sugerir confirmação de consulta nem automação. Use o mesmo vestuário azul profundo e, se a personagem da referência for sintética, preserve a continuidade dessa personagem.

COMPOSIÇÃO
Enquadramento médio, em três quartos, com rosto e mãos visíveis dentro dos 70% centrais. O aparelho é um detalhe do gesto; não mostrar marcas, mensagens, agenda, dados de pacientes ou telas legíveis. A luz vem lateralmente e revela a textura natural da madeira. Preserve espaço ao redor para recorte quadrado sem cortar mãos ou cabeça.
...
- A cena comunica “facilitar o contato” sem precisar de uma legenda técnica?
...
Salve a variante como `04-contato-mobile.webp`. A primeira versão da LP usa recorte CSS; a variante mobile requer inclusão em um elemento `<picture>` quando integrada.
